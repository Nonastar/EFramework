import { Asset, AssetManager, Font, ImageAsset, JsonAsset, Label, SceneAsset, Sprite, SpriteFrame, Texture2D, TextureCube, assetManager, isValid, path, sp } from 'cc';
import { DEV, MINIGAME } from 'cc/env';
const REGEX = /^https?:\/\/.*/;
...
export class Loader {
    ...
    protected get log() {
        if (DEV) {
            return window.console.log.bind(window.console,
                ...
                `[Loader] LOG ${new Date().toLocaleString()}`,
    ...
    protected get warn() {
        if (DEV) {
            return window.console.warn.bind(window.console,
                ...
                `[Loader] WARN ${new Date().toLocaleString()}`,
    ...
    protected get error() {
        if (DEV) {
            return window.console.error.bind(window.console,
                ...
                `[Loader] ERROR ${new Date().toLocaleString()}`,
    ...
    private handle(handle: string, { bundle, version, path, type, onProgress, onComplete }: { bundle?: string, version?: string, path: string, type?: typeof Asset, onProgress?: Action<number>, onComplete?: Action<unknown> }) {
        if (!handle) {
        ...
        if (onProgress) {
            let progress = 0;
            args.push((finish: number, total: number) => {
     ...
     */
    public preloadAsset(params: { path: string, bundle?: string, version?: string, type?: typeof Asset, onProgress?: Action<number>, onComplete?: Action<AssetManager.RequestItem[] | null> }) {
     ...
     * @param params.version 远程bundle的版本，参考https://docs.cocos.com/creator/manual/zh/asset/bundle.html#asset-bundle-%E7%9A%84%E7%89%88%E6%9C%AC
     */
    public preloadDir(params: { path: string, bundle?: string, version?: string, type?: typeof Asset, onProgress?: Action<number>, onComplete?: Action<AssetManager.RequestItem[] | null> }) {
     ...
     */
    public loadAsset<T extends typeof Asset>(params: { path: string, bundle?: string, version?: string, type?: T, onProgress?: Action<number>, onComplete?: Action<InstanceType<T> | null> }) {
        if (SceneAsset === params.type as typeof Asset) {
     ...
     * @param params.type 资源类型
     */
    public loadAssetAsync<T extends typeof Asset>(params: { path: string, bundle?: string, version?: string, type?: T, onProgress?: Action<number> }): Promise<InstanceType<T> | null> {
     ...
     * @param params.type 资源类型
     */
    public loadDir<T extends typeof Asset>(params: { path: string, bundle?: string, version?: string, type?: T, onProgress?: Action<number>, onComplete?: Action<InstanceType<T>[] | null> }) {
     ...
     * @param params.type 资源类型
     */
    public loadDirAsync<T extends typeof Asset>(params: { path: string, bundle?: string, version?: string, type?: T, onProgress?: Action<number> }): Promise<InstanceType<T>[] | null> {
        return new Promise((resolve) => {
            this.loadDir({
                ...params,
                onComplete: resolve
     ...
     * @param bundle 默认为resources，如果是远程bundle，则使用url末位作为bundle名
     */
    public unloadBundle(bundle?: string) {
        if (!bundle) bundle = 'resources';
    ...
    public reloadBundle({ bundle, version, onComplete }: { bundle?: string, version?: string, onComplete?: (bundle: AssetManager.Bundle | null) => any }) {
        ...
        assetManager.loadRemote(configUrl, (err: Error, data: JsonAsset) => {
            ...
            this.unloadBundle(path.basename(bundle));
