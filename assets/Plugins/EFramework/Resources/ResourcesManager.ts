...
class ResourcesManager extends Singleton {
	constructor() {
		...
		// 定时自动释放资源
		if (ResourcesManager._config.cacheLifetimeMsNum !== 0) {
			this._releaseTimer = setInterval(this._autoReleaseAsset.bind(this), ResourcesManager._config.cacheLifetimeMsNum);
		}
	...
	/* --------------- private --------------- */
	/** 日志 */
	private _log = new ELogger("Resources");
	/** 管理表 */
	...
	loadAsset<T extends Asset>(path: string, type_: Constructor<T>, target_: MKRelease_.TypeFollowReleaseSupport, config_?: MKAsset_.GetConfig<T> ): Promise<T | null> {
		...
		// 参数转换
		{
			...
			if (path.startsWith("db://assets/")) {
				...
				if (!(EDITOR && !window["cc"].GAME_VIEW) && path.includes("/")) {
			...
			if (!(EDITOR && !window["cc"].GAME_VIEW) && !isRemote) {
				const indexNum = path.lastIndexOf(".");

				if (indexNum !== -1) {
					path = path.slice(0, indexNum);
				}
			}
			...
			if (!isRemote) {
				...
				if (assetType === SpriteFrame && !path.endsWith("/spriteFrame")) {
					path += "/spriteFrame";
				} else if (assetType === Texture2D && !path.endsWith("/texture")) {
					path += "/texture";
				}
			}
		}
		...
		return new Promise<T | null>(async (resolveFunc) => {
			...
			const completedFunc = async (error: Error | null, asset: T): Promise<void> => {
				...
				if (error) {
					...
					// 重试
						...
						this._log.warn(`重新加载 get ${path} 错误，剩余重试次数 ${getConfig.retryNum}`);
						getConfig.retryNum!--;
						resolveFunc(await this.loadAsset(path, type_, target_, getConfig));
					}
			...
			// 编辑器
				...
				// 补全加载配置
				{
					...
					// uuid
					{
						...
						if (!uuidStr) {
							this._log.error("获取 uuid 失败", path);
							getConfig.completedFunc?.(new Error("获取 uuid 失败，" + path), null!);
							resolveFunc(null);
			...
			// 本地
				...
				// 获取资源
				const asset = bundleAsset.get(path, type_);
				...
				// 加载资源
					...
					if (getConfig.progressFunc) {
						bundleAsset.load(path, type_, getConfig.progressFunc, completedFunc);
					} else {
						bundleAsset.load(path, type_, completedFunc);
					}
				}
			}
		});
	...
	private _autoReleaseAsset(isForce_ = false): void {
		...
		if (isForce_) {
			...
			for (const [kStr, v] of this._assetReleaseMap.entries()) {
				// 当前及之后的资源没超过生命时长
				if (currentTimeMsNum - v.joinTimeMsNum < ResourcesManager._config.cacheLifetimeMsNum) {
					break;
				}
