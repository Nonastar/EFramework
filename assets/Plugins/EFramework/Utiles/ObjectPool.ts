...
/** 异步对象池 */
class ObjectPool<CT> {
	constructor(init_: _ObjectPool.Config<CT>) {
		this.config = new _ObjectPool.Config(init_);
	...
	/* --------------- public --------------- */
	/** 初始化数据 */
	config!: _ObjectPool.Config<CT>;
	/** 初始化任务 */
	initTask = new ETask(false);
	/** 有效状态 */
	get isValid(): boolean {
...
namespace ObjectPool {
	...
	export class Sync<CT> {
		constructor(init_?: _ObjectPool.Sync.Config<CT>) {
			this.config = new _ObjectPool.Sync.Config(init_);
			if (this.config.initFillNum! > 0) {
		...
		/* --------------- public --------------- */
		/** 初始化数据 */
		config!: _ObjectPool.Sync.Config<CT>;
		/** 有效状态 */
		get isValid(): boolean {
			return this._isValid;
		}
