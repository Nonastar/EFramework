...
export class TimeTools {
    ...
    public static _configBoot(): void {
        ...
        eLog.log("系统启动时间", this._osBootTime);
        TimeCache = new Date();
    ...
    public static setNetTime(netTime: number): void {
        ...
        eLog.log(`设置网络时间: net(${this.formatTime(this.netTime)}), boot(${this.formatTime(this.osBootTime)}), diff(${Math.abs(this.netTimeDiff / 1000)}秒)`);
    }
    ...
    public static getWeekDay(time?: number): number {
        TimeCache.setTime(time || TimeTools.now());
