...
abstract class MKNetworkBase<CT extends MKCodecBase = MKCodecBase> extends Singleton {
	...
	protected _open(event_: any): void {
		...
		console.debug("socket 准备完成", event_);
	...
	protected _error(event_: any): void {
		if (this._state === MKNetworkBase_.Status.Open) {
			console.error("socket 错误", event_);
		}
	}
	...
	protected _close(event_: any): void {
		...
		if (this._reconnectTimer !== null) {
			...
			console.warn("socket 重连计数", this._reconnectCountNum);
		}

		// 准备重连
		if (lastStatus === MKNetworkBase_.Status.Open && this._reconnectTimer === null) {
			console.warn("socket 开始重连");
	...
	protected _cancelReconnect(isStatus_: boolean): void {
		...
		console.warn("socket 重连" + (isStatus_ ? "成功" : "失败"));
