...
import ObjectPool from "../Utiles/ObjectPool";
 ...
 * - 全屏 UI 展示优化
 */
export class UIManage extends Singleton {
	constructor() {
		super();
	...
	/* --------------- public --------------- */
	/** 事件 */
	event = new EEvent<_UIManage.EventProtocol>();
	/**
	 * 获取模块注册数据
	 * @remarks
	 * open 未注册模块时会使用此函数获取注册数据自动注册
	 */
	getRegisDataFunc?: <T extends Constructor<MKViewBase>>(key: T) => UIManage_.RegisData<T>;
	/* --------------- private --------------- */
	/** 日志 */
	private _log = new ELogger("UIManage");
	/** 模块注册表 */
	private _uiRegisMap = new Map<any, UIManage_.RegisData<any>>();
	/**
	 * 模块注册任务表
	 * @remarks
	 * 用于 open 时等待注册
	 */
	private _uiRegisTaskMap = new Map<any, ETask>();
	/**
	 * 模块加载表
	 * @remarks
	 * 用于检测重复加载
	 */
	private _uiLoadMap = new Map<any, ETask>();
	/** 模块对象池 */
	private _uiPoolMap = new Map<any, Map<string, ObjectPool<Node>>>();
	/** 隐藏模块列表长度 */
	...
	async regis<T extends Constructor<MKViewBase>>(
		key_: T,
		source_: _UIManage.TypeRegisSource<T>,
		...
		/** 注册数据 */
		const regisData = new UIManage_.RegisData<T>({
			...config_,
			source: source_,
		});

		uiRegisTask = new ETask(false);
		// 添加注册任务
		...
		/** 节点池 */
		const objectPoolMap = new Map<string, ObjectPool<Node>>();
		...
		for (const kStr in sourceTab) {
			...
			/** 对象池 */
			const objectPool = new ObjectPool<Node>({
				createFunc: async () => {
					// 不存在预制体开始加载
	 ...
	 * @param type_ 模块类型
	 */
	get<T extends UIManage_.TypeOpenKey, T2 = _UIManage.TypeModule<T>, T3 = T["prototype"]>(key_: T, type_?: T2): T3 | null;
	/**
	 * 获取指定模块列表
	 * @param key_ 模块键列表 [type]
	 * @param type_ 模块类型
	 */
	get<T extends UIManage_.TypeOpenKey, T2 = _UIManage.TypeModule<T>, T3 = T["prototype"]>(key_: T[], type_?: T2): ReadonlyArray<T3>;
	get<T extends UIManage_.TypeOpenKey, T2 = _UIManage.TypeModule<T>, T3 = T["prototype"]>(
		key_?: T | T[],
		type_?: T2
	 ...
	 * @returns
	 */
	async open<T extends UIManage_.TypeOpenKey, T2 = T["prototype"]>(key_: T, config_?: UIManage_.OpenConfig<T>): Promise<T2 | null> {
		if (!key_) {
		...
		config_ = new UIManage_.OpenConfig(config_);
		...
		// 检测重复加载
		{
			...
			// 首次加载
			if (!task) {
				this._uiLoadMap.set(key_, (task = new ETask(false)));
			}
			// 再次加载
			else {
				if (
	 ...
	 * @returns
	 */
	async close<T extends Constructor<MKViewBase>, T2 extends MKViewBase>(args_: Node | T | T2): Promise<boolean> {
		if (!args_) {
			this._log.error("参数错误");
		...
		for (const v of closeUIList) {
			...
			// 销毁
			if ( v.isStatic || !this._uiPoolMap.has(v.constructor)) {
				v.node.destroy();
			}
			// 回收模块
			else {
...
export namespace UIManage_ {
	...
	export class OpenConfig<CT extends UIManage_.TypeOpenKey> {
		...
		/** 类型 */
		type?: _UIManage.TypeModule<CT> = "default";
		/** 父节点 */
		parent?: Node | null;
	}
