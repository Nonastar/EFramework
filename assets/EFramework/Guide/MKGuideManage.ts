import ETaskPipeline from "../Core/Task/ETaskPipeline";
...
class MKGuideManage {
	...
	/* --------------- public --------------- */
	/** 事件 */
	event = new EEvent<MKGuideManage_.EventProtocol>();
	/** 步骤表 */
	...
	private _stepNum = -1;
	/** 任务管线 */
	private _taskPipeline = new ETaskPipeline();
	/** 步骤预加载任务表 */
	...
	run(): Promise<void> {
		return this._taskPipeline.add(async () => {
			...
			// 步骤未注册
			if (!currentStep) {
				console.error(`步骤 ${this._stepNum} 未注册`);
				this.isPause = true;
			...
			// 加载步骤事件
			console.log("执行步骤", currentStep.stepNum, currentStep.describeStr ?? "");
	...
	setStep(stepNum_: number, initData_?: any): Promise<void> {
		return this._taskPipeline.add(async () => {
			...
			console.log("切换到步骤", this._stepNum, currentStep?.describeStr ?? "");
	...
	private _updateStepData(): boolean {
		...
		if ((stepData ?? null) === null) {
			...
			console.warn(`当前步骤 ${this._stepNum} 数据错误，引导中断`);
		...
		if (currentStep) {
			...
			if (this._stepNum !== this._initConfig.endStepNum) {
				console.warn(`当前步骤 ${this._stepNum} 未注册，引导中断`);
			}
